  ...
  methods: {
    ...
    init() {
      if (this.$route.query.id) {
        this.shangpinid = this.$route.query.id;
      }
      if (this.$route.query.jine) {
        this.ruleForm.jine = this.$route.query.jine;
      }
    ...
    onSubmit() {
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.ruleForm.jine = parseInt(this.ruleForm.jine);
          this.$http.post('dingdan/add?shangPinId=' + this.shangpinid, this.ruleForm).then(res => {
            if (res.data.code == 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
