	...
	<div>
		<div class="breadcrumb-preview dy-breadcrumb-div">
			<el-breadcrumb :separator="'Ξ'"
				...
				<el-breadcrumb-item>信息介绍列表</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		...
		<div class="list-preview"
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv dy-list-form">
				<el-form-item v-if="!isQuery.leibie" :style='{ "margin": "0 10px" }'>
					<div v-if="true" class="lable" :style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "display": "inline-block" }'>类别</div>
					<el-input suffix-icon="el-icon-search" v-model="formSearch.leibie" placeholder="类别" clearable></el-input>
				</el-form-item>
			...
			<div class="list" :style='{ "width": "100%", "margin": "0", "background": "none", "order": "3" }'>
				<div class="table-content dy-list-div">
					<el-table class="tables" size="medium" :show-header="true" header-row-class-name="dy-list-el-table-header-row" header-cell-class-name="dy-list-el-table-header-cell" :style="{ width: '100%', fontSize: '14px', color: 'rgba(72, 72, 72, 1)' }" :data="dataList" :stripe="true" :highlight-current-row="true" tooltip-effect="dark">
						...
						<el-table-column align="center" header-align="center" label="类别" :show-overflow-tooltip="true">
...
export default {
	...
	methods: {
		...
		getList() {
			...
			let searchWhere = {};
			if (this.formSearch.leibie != '' && this.formSearch.leibie != undefined) {
				searchWhere.leibie = this.formSearch.leibie;
			}

			if (this.$route.query.leibie) {
				searchWhere.leibie = this.$route.query.leibie;
				this.formSearch.leibie = this.$route.query.leibie;
				this.isQuery.leibie = true;
			} else {
				this.isQuery.leibie = false;
			}


			this.$http.get('dx/manaqian', { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
		...
		toDetail(id) {
			this.$router.push({ path: '/index/dxDetail', query: { id: id } });
		},
