	...
	<div>
		<div class="breadcrumb-preview dy-breadcrumb-div">
			<el-breadcrumb :separator="'Ξ'"
				...
				<el-breadcrumb-item>我的收藏列表</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		getList() {
			...
			this.$http.get('shoucang/manamyqian', { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
		...
		toDetail(id) {
			this.$router.push({ path: '/index/shoucangDetail', query: { id: id } });
		},
		//获取所有零食
		getShangpinList() {
