<template>
  <div class="main-content">
        ...
        <el-row class="ad" style="justify-content:flex-start;">
          <el-form-item>
            <el-button :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete"
              @click="xinwenDeleteHandler()">批量删除</el-button>
          </el-form-item>
        </el-row>
      </el-form>
      <!--数据列表区域-->
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column width="350" :align="contents.tableAlign" header-align="center" label="操作" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="success" icon="el-icon-info" size="mini" @click="xinwenDetailHandler(scope.row.id)">详情</el-button>
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="xinwenSetHandler(scope.row.id)">修改</el-button>
              <el-button type="danger" icon="el-icon-delete" size="mini" @click="xinwenDeleteHandler(scope.row.id)">删除</el-button>
    ...
    <!-- 新闻修改-->
    <xinwen-set v-if="xinwenSetFlag" :parent="this" ref="xinwenSet"></xinwen-set>
    <!-- 新闻详情-->
    <xinwen-detail v-if="xinwenDetailFlag" :parent="this" ref="xinwenDetail"></xinwen-detail>

  </div>
</template>
<script>
import XinwenSet from "./xinwen-set"
import XinwenDetail from "./xinwen-detail"
...
export default {
  ...
  methods: {
    ...
    //跳转到新闻详情页面
    xinwenDetailHandler(id) {
      this.showFlag = false;
      this.xinwenDetailFlag = true;
      this.$nextTick(() => {
        this.$refs.xinwenDetail.init(id);
      });
    ...
    xinwenSetHandler(id) {
      this.showFlag = false;
      this.xinwenSetFlag = true;
      this.$nextTick(() => {
        this.$refs.xinwenSet.init(id);
      });
    },

    //更新新闻信息操作方法
    xinwenSet(tid,tname,property,value) {
      let params = {};
      params['id'] = tid;
