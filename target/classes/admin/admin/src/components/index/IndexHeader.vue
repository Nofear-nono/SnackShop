<template>
	<div class="navbar" :style="{backgroundColor:heads.headBgColor,height:heads.headHeight,boxShadow:heads.headBoxShadow,lineHeight:heads.headHeight}">
		...
		<div class="right-menu">
			<div class="user-info" :style="{color:heads.headUserInfoFontColor,fontSize:heads.headUserInfoFontSize}">{{this.$storage.get('typename')}} {{this.$storage.get('adminName')}}</div>
			<div v-if="users.money" class="user-info" style="color: skyblue;font-size: 16px;">帐号余额：￥{{users.money}}</div>
