<template>
  <div class="main-content">
        ...
        <el-row class="ad" style="justify-content:flex-start;">
          <el-form-item>
            <el-button :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete"
              @click="pinleiDeleteHandler()">批量删除</el-button>
          </el-form-item>
        </el-row>
      </el-form>
      <!--数据列表区域-->
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column width="350" :align="contents.tableAlign" header-align="center" label="操作" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="success" icon="el-icon-info" size="mini" @click="pinleiDetailHandler(scope.row.id)">详情</el-button>
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="pinleiSetHandler(scope.row.id)">修改</el-button>
              <el-button type="danger" icon="el-icon-delete" size="mini" @click="pinleiDeleteHandler(scope.row.id)">删除</el-button>
              <!-- <el-button type="success" icon="el-icon-tickets" size="mini" @click="shangpinChakanHandler(scope.row.id)">零食查看</el-button> -->
              <!-- <el-button type="success" icon="el-icon-circle-plus" size="mini" @click="shangpinAddHandler(scope.row.id, scope.row.mingcheng)">零食添加</el-button> -->
    ...
    <!-- 分类修改-->
    <pinlei-set v-if="pinleiSetFlag" :parent="this" ref="pinleiSet"></pinlei-set>
    <!-- 分类详情-->
    <pinlei-detail v-if="pinleiDetailFlag" :parent="this" ref="pinleiDetail"></pinlei-detail>
    <!-- 分类零食查看-->
    <pinlei-shangpin-chakan v-if="pinleiShangpinChakanFlag" :parent="this" ref="pinleiShangpinChakan"></pinlei-shangpin-chakan>
    <!-- 分类零食添加-->
    <pinlei-shangpin-add v-if="pinleiShangpinAddFlag" :parent="this" ref="pinleiShangpinAdd"></pinlei-shangpin-add>

  </div>
</template>
<script>
import PinleiSet from "./pinlei-set"
import PinleiDetail from "./pinlei-detail"
import PinleiShangpinChakan from './pinlei-shangpin-chakan'
import PinleiShangpinAdd from './pinlei-shangpin-add'
...
export default {
  data() {
    return {
      ...
      showFlag: true,
      pinleiSetFlag: false,
      pinleiDetailFlag: false,
      pinleiShangpinChakanFlag: false,
      pinleiShangpinAddFlag: false,
  ...
  methods: {
    ...
    //跳转到分类详情页面
    pinleiDetailHandler(id) {
      this.showFlag = false;
      this.pinleiDetailFlag = true;
      this.$nextTick(() => {
        this.$refs.pinleiDetail.init(id);
      });
    ...
    pinleiSetHandler(id) {
      this.showFlag = false;
      this.pinleiSetFlag = true;
      this.$nextTick(() => {
        this.$refs.pinleiSet.init(id);
      });
    },

    //更新分类信息操作方法
    pinleiSet(tid,tname,property,value) {
      let params = {};
      params['id'] = tid;
    ...
    shangpinAddHandler(id, mingcheng) {
      this.showFlag = false;
      this.pinleiShangpinAddFlag = true;
      this.$nextTick(() => {
        this.$refs.pinleiShangpinAdd.init(id, mingcheng);
      });
    },
    ...
    shangpinChakanHandler(id) {
      this.showFlag = false;
      this.pinleiShangpinChakanFlag = true;
      this.$nextTick(() => {
        this.$refs.pinleiShangpinChakan.init(id);
      });
    },
