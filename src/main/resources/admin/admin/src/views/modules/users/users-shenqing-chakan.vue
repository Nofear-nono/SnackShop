    ...
    <div v-if="showFlag">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content" v-if="false">
        <el-row :gutter="20" class="slt" style="justify-content:flex-start;">
          ...
          <el-form-item>
            <el-select v-model="searchForm.shenhe" placeholder="请选择是否审核">
        ...
        <!-- <el-row class="ad" style="justify-content:flex-start;">
          <el-form-item>
            <el-button :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete"
              @click="shenqingDeleteHandler">批量删除</el-button>
          </el-form-item>
        </el-row> -->
      </el-form>
      <!--数据列表区域-->
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column :align="contents.tableAlign" header-align="center" label="是否审核">
            <template slot-scope="scope">
              {{ scope.row.shenhe }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="350" :align="contents.tableAlign" header-align="center" label="操作" fixed="right">
            <template slot-scope="scope">
              <!-- <el-button type="primary" icon="el-icon-edit-outline" size="mini" @click="shenqingSet(scope.row.id,scope.row.shenhe == 'no' ? '是否审核' : '取消是否审核','shenhe',scope.row.shenhe == 'no' ? 'yes' : 'no')">{{scope.row.shenhe == 'no' ? '是否审核' : '取消是否审核' }}</el-button> -->


              <el-button type="success" icon="el-icon-info" size="mini" @click="shenqingDetailHandler(scope.row.id)">详情</el-button>
              <!-- <el-button type="primary" icon="el-icon-edit" size="mini" @click="shenqingSetHandler(scope.row.id)">修改</el-button> -->
              <!-- <el-button type="danger" icon="el-icon-delete" size="mini" @click="shenqingDeleteHandler(scope.row.id)">删除</el-button> -->
        ...
        <el-button class="btn-close" style="width: 88px; height: 44px; color: rgb(255, 255, 255); font-size: 14px; border-width: 0px; border-style: solid; border-color: rgb(64, 158, 255); border-radius: 4px; background-color: rgb(252, 201, 51);" @click="back()">返回</el-button>
      </div>
    </div>
    <!-- 售后申请修改-->
    <shenqing-set v-if="shenqingSetFlag" :parent="this" ref="shenqingSet"></shenqing-set>
    <!-- 售后申请详情-->
    <shenqing-detail v-if="shenqingDetailFlag" :parent="this" ref="shenqingDetail"></shenqing-detail>
  </div>
</template>
<script>
import ShenqingSet from "../shenqing/shenqing-set"
import ShenqingDetail from "../shenqing/shenqing-detail"
export default {
  data() {
    return {
      searchForm: {
      ...
      dataListSelections: [],
      showFlag: true,
      shenqingSetFlag: false,
      shenqingDetailFlag: false,
  ...
  methods: {
    ...
    back() {
      ...
      this.parent.usersShenqingChakanFlag = false;
    ...
    //跳转到售后申请详情页面
    shenqingDetailHandler(id) {
      this.showFlag = false;
      this.shenqingDetailFlag = true;
      this.$nextTick(() => {
        this.$refs.shenqingDetail.init(id);
      });

    },

    //跳转到售后申请修改页面
    shenqingSetHandler(id) {
      this.showFlag = false;
      this.shenqingSetFlag = true;
      this.$nextTick(() => {
        this.$refs.shenqingSet.init(id);
      });
    },

    //更新售后申请信息操作方法
    shenqingSet(tid,tname,property,value) {
      let params = {};
      params['id'] = tid;
