	...
	<div>
		...
		<div class="list-preview"
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv dy-list-form">
				<el-form-item v-if="!isQuery.name" :style='{ "margin": "0 10px" }'>
					<div v-if="true" class="lable" :style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "display": "inline-block" }'>名称</div>
					<el-input suffix-icon="el-icon-search" v-model="formSearch.name" placeholder="名称" clearable></el-input>
				</el-form-item>
				<el-form-item v-if="!isQuery.url" :style='{ "margin": "0 10px" }'>
					<div v-if="true" class="lable" :style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "display": "inline-block" }'>链接地址</div>
					<el-input suffix-icon="el-icon-search" v-model="formSearch.url" placeholder="链接地址" clearable></el-input>
				</el-form-item>
			...
			<div class="list" :style='{ "width": "100%", "margin": "0", "background": "none", "order": "3" }'>
				<div class="table-content dy-list-div">
					<el-table class="tables" size="medium" :show-header="true" header-row-class-name="dy-list-el-table-header-row" header-cell-class-name="dy-list-el-table-header-cell" :style="{ width: '100%', fontSize: '14px', color: 'rgba(72, 72, 72, 1)' }" :data="dataList" :stripe="true" :highlight-current-row="true" tooltip-effect="dark">
						...
						<el-table-column align="center" header-align="center" label="链接地址" :show-overflow-tooltip="true">
							<template slot-scope="scope">
								{{ scope.row.url }}
							</template>
						</el-table-column>
...
export default {
	...
	methods: {
		...
		getList() {
			...
			let searchWhere = {};
			if (this.formSearch.name != '' && this.formSearch.name != undefined) {
				searchWhere.name = this.formSearch.name;
			}
			if (this.formSearch.url != '' && this.formSearch.url != undefined) {
				searchWhere.url = this.formSearch.url;
			}

			if (this.$route.query.name) {
				searchWhere.name = this.$route.query.name;
				this.formSearch.name = this.$route.query.name;
				this.isQuery.name = true;
			} else {
				this.isQuery.name = false;
			}

			if (this.$route.query.url) {
				searchWhere.url = this.$route.query.url;
				this.formSearch.url = this.$route.query.url;
				this.isQuery.url = true;
			} else {
				this.isQuery.url = false;
			}


			this.$http.get('lianjie/manaqian', { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
		...
		toDetail(id) {
			this.$router.push({ path: '/index/lianjieDetail', query: { id: id } });
		},
