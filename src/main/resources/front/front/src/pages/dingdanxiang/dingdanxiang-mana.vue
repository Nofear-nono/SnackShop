	...
	<div>
		<div class="breadcrumb-preview dy-breadcrumb-div">
			<el-breadcrumb :separator="'Ξ'"
				...
				<el-breadcrumb-item>订单项列表</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		...
		<div class="list-preview"
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv dy-list-form">
				<el-form-item v-if="!isQuery.dingdanid" :style='{ "margin": "0 10px" }'>
					<div v-if="true" class="lable" :style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "display": "inline-block" }'>订单</div>
					<el-select v-model="formSearch.dingdanid" placeholder="请选择订单" :clearable="true">
				...
				<el-form-item v-if="!isQuery.goumaishuliang" :style='{ "margin": "0 10px" }'>
			...
			<div class="list" :style='{ "width": "100%", "margin": "0", "background": "none", "order": "3" }'>
				<div class="table-content dy-list-div">
					<el-table class="tables" size="medium" :show-header="true" header-row-class-name="dy-list-el-table-header-row" header-cell-class-name="dy-list-el-table-header-cell" :style="{ width: '100%', fontSize: '14px', color: 'rgba(72, 72, 72, 1)' }" :data="dataList" :stripe="true" :highlight-current-row="true" tooltip-effect="dark">
						...
						<el-table-column align="center" header-align="center" label="订单" :show-overflow-tooltip="true">
						...
						<el-table-column align="center" header-align="center" label="商品数量" :show-overflow-tooltip="true">
...
export default {
	...
	methods: {
		...
		getList() {
			...
			let searchWhere = {};
			if (this.formSearch.dingdanid != '' && this.formSearch.dingdanid != undefined) {
				searchWhere.dingdanid = this.formSearch.dingdanid;
			}
			if (this.formSearch.shangpinid != '' && this.formSearch.shangpinid != undefined) {
				searchWhere.shangpinid = this.formSearch.shangpinid;
			}
			if (this.formSearch.goumaishuliang != '' && this.formSearch.goumaishuliang != undefined) {
				searchWhere.goumaishuliang = this.formSearch.goumaishuliang;
			}
			if (this.formSearch.usersid != '' && this.formSearch.usersid != undefined) {
				searchWhere.usersid = this.formSearch.usersid;
			}

			if (this.$route.query.dingdanid) {
				searchWhere.dingdanid = this.$route.query.dingdanid;
				this.formSearch.dingdanid = this.$route.query.dingdanid;
				this.isQuery.dingdanid = true;
			} else {
				this.isQuery.dingdanid = false;
			}
			...
			if (this.$route.query.goumaishuliang) {
				searchWhere.goumaishuliang = this.$route.query.goumaishuliang;
				this.formSearch.goumaishuliang = this.$route.query.goumaishuliang;
				this.isQuery.goumaishuliang = true;
			} else {
				this.isQuery.goumaishuliang = false;
			}
			...
			this.$http.get('dingdanxiang/manaqian', { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
		...
		toDetail(id) {
			this.$router.push({ path: '/index/dingdanxiangDetail', query: { id: id } });
		},
		//获取所有订单
		getDingdanList() {
