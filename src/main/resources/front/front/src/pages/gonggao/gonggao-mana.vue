						...
						<el-table-column align="center" header-align="center" label="图片">
							<template slot-scope="scope">
								<div v-if="scope.row.tupian">
									<img :src="baseUrl + scope.row.tupian" width="80" height="80">
								</div>
								<div v-else>暂无图片</div>
							</template>
						</el-table-column>
...
export default {
	...
	methods: {
		...
		getList() {
			...
			this.$http.get('gonggao/manaqian', { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
		...
		toDetail(id) {
			this.$router.push({ path: '/index/gonggaoDetail', query: { id: id } });
		},
